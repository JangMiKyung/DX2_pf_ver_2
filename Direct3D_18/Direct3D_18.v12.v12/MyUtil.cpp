...
namespace MyUtil{
	...
	float GetAngle(D3DXVECTOR3& vec1, D3DXVECTOR3& vec2)
	{
		D3DXVECTOR3 v1;
		D3DXVECTOR3 v2;
		D3DXVec3Normalize(&v1, &vec1);
		D3DXVec3Normalize(&v2, &vec2);
		float len = (float)(D3DXVec3Length(&vec1) * D3DXVec3Length(&vec2));
		float result = D3DXVec3Dot(&vec1, &vec2);
		float a = acosf (ClampMinusOnePlusOne(D3DXVec3Dot(&vec1, &vec2) / (float)(D3DXVec3Length(&vec1) * D3DXVec3Length(&vec2))));
