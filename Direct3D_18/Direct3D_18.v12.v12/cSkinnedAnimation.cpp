...
HRESULT	cSkinnedAnimation::Init(cXMesh_Skinned* pSkinnedMesh)
{
	...
	this->Play(0);
	m_viAnimeSet = m_vecComAnimSet.begin();
}
...
void cSkinnedAnimation::PlayOneShot(LPD3DXANIMATIONSET animName, float inCrossFadeTime, float outCrossFadeTime)
{
	m_bPlay = true;
	m_bLoop = false;
	isAniFinish = false;
...
void cSkinnedAnimation::PlayAniCombo()
{
	if (isComboAni)
	{
		PlayOneShot((*m_viAnimeSet), 3.0f);

		if (!isAniFinish)
		{
			m_viAnimeSet++;
			if (m_viAnimeSet == m_vecAnimSet.end())
			{
				isComboAni = false;
				m_viAnimeSet = m_vecComAnimSet.begin();
			}
...
void cSkinnedAnimation::pushAniCombo(string Anime)
{
	MAPITER_ANIMSET ani;
	ani = m_mapAnimSet.find(Anime);

	if(ani != m_mapAnimSet.end())
	m_vecComAnimSet.push_back((*ani).second);
}
