...
HRESULT cScene_03::Init()
{
	...
	D3DXCreateTextureFromFile(
		Device,					//디바이스
		"Taegina.png",			//경로포함한 파일명
		&this->m_pTex			//얻어올 IDirect3DTexture9 더블포인터
		);

	D3DXCreateTextureFromFile(
		Device,					//디바이스
		"TaeginaMask2.png",			//경로포함한 파일명
		&this->m_pTex2			//얻어올 IDirect3DTexture9 더블포인터
		);
	...
	D3DXCreateEffectFromFile(
		Device,						//디바이스
		"Texure_Mask2.fx",			//불러올 셰이더 코드 파일이름
...
void cScene_03::Render()
{
	...
	this->pEffect->SetVector("vColor1", &D3DXVECTOR4(1, 0, 1, 1));
	this->pEffect->SetVector("vColor2", &D3DXVECTOR4(0, 1, 1, 1));
	this->pEffect->SetVector("vColor3", &D3DXVECTOR4(0, 1, 0, 1));


	//Texture 셋팅
	this->pEffect->SetTexture("MyTex", m_pTex);
	this->pEffect->SetTexture("MaskTex", m_pTex2);

	//타일수 셋팅
	this->pEffect->SetFloat("tilingX", 1);
	this->pEffect->SetFloat("tilingY", 1);
