...
void cScene_02::Init()
{
	...
	//this->camPos.AttachTo( &this->cubeTrans );
	this->camPos.SetLocalPosition(0.0f, 1, -5);
	...
	float Rand = RandomFloatRange(-3.0f, 3.0f);
}
...
void cScene_02::Update(float timeDelta)
{
	...
	mainCamera.Interpolate(mainCamera, this->camPos, timeDelta * 1.0f);
	...
	//캠진동
	{
		...
		if (shakePower > 0.0f)
		{
			...
			//shakePos.z = RandomFloatRange( -shakePower, shakePower );
			shakePos.z = 0.0f;


			LOG_MGR->AddLog("%.2f, %.2f, %.2f",
				shakePos.x,
				shakePos.y,
				shakePos.z);

			shakePower -= timeDelta;
			if (shakePower < 0.0f)
				shakePower = 0.0f;
		}
		else
		...
		this->cubeTrans.SetWorldPosition(shakePos);
...
void cScene_02::Render()
{
	...
	this->camPos.RenderGimozo();
}

void cScene_02::RenderCube(){
